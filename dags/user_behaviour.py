...
extract_user_purchase_data = PostgresOperator(  # PostgresOperator is used to execute SQL code in a specific Postrgres Database
    dag=dag,  # Assign task to dag
    task_id="extract_user_purchase_data",  # Name of task
    sql="./scripts/sql/unload_user_purchase.sql",  # SQL code to query data from database
    postgres_conn_id="postgres_default",  # Reference to a specific database
    params={
        "user_purchase": "/temp/user_purchase.csv"
    },  # A value for params in the sql scripts, which is the path of the csv file
    depends_on_past=True,  # To make task run sequentially (run one by one)
    wait_for_downstream=True,  # To make task run sequentially (run one by one)
)

user_purchase_to_stage_data_lake = PythonOperator(  # PythonOperator is used to execude Python callable (something that can be called)
    dag=dag,
    task_id="user_purchase_to_stage_data_lake",
    python_callable=_local_to_s3,  # A reference to the callable python code that is called
    op_kwargs={
